...
let simToDo = {
    ...
    toDoInputId:'todoInput',
    toDoContainerId:'todoListContainer',
    ...
    createUISkeleton(){
        this.targetContainer = document.querySelector(`#${this.mainContainerId}`)?? document.body;
        this.targetContainer.style.display = 'flex';
        this.targetContainer.style.flexDirection= 'row';
    },
    createToDoElements(){
        let toDoInput = this.createElm('div');
            toDoInput.id=this.toDoInputContainerId;

        let toDoContainer = this.createElm('div');
            toDoContainer.id=this.toDoContainerId;
        let headerText = this.createElm('h2');
        let toDoBody = this.createElm('textarea');
            toDoBody.classList.add('simToDo-toDoInput')
            toDoBody.id=this.toDoInputId;
        ...
        saveToDoButton.textContent=this.textLabels[this.activeState.lang].saveButton;
        headerText.textContent=this.textLabels[this.activeState.lang].headerText;
    ...
    prepareNewToDoObject(){
        let toDoBody = document.getElementById(this.toDoInputId);
        if(!toDoBody.value){
            this.giveMessage({
                type: 'error',
    ...
    clearToDoInput(){
        document.getElementById(this.toDoInputId).value='';
    },
    drawFilters(){
        let filterOptionsContainer = this.createFilterOptionsContainer();

    },
    drawToDos(pageNo=1){
        let todoContainer = document.getElementById(this.toDoContainerId);
